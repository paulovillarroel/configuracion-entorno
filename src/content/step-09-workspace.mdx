...
```
      ...
      **Opción B: Visual (Arrastrar)**
      ...
      <Callout type="warning" title="Importante">
        Si copias un proyecto de código, borra las carpetas `node_modules` y `.venv` y genéralas de nuevo en Linux: contienen binarios compilados para Windows que fallarán. **No borres la carpeta `.git`** — el historial de Git es portable entre sistemas y borrarlo eliminaría todos tus commits y la conexión con el repositorio remoto.
      </Callout>
    </div>
  </Fragment>
  <Fragment slot="mac">
...
Con tu `workspace` listo, el flujo para cualquier proyecto nuevo es siempre el mismo:
...
Entras a tu carpeta maestra, creas una carpeta para el proyecto, entras en ella y la abres en Positron. Repetirás este patrón cada vez que empieces algo nuevo.

<Callout type="info" title="Si positron . no abre nada">
  **Positron se instala y configura más adelante, en [IDE & Verificación](#paso-12).** Si aún no completaste ese paso, `positron .` todavía no funcionará: crea la carpeta del proyecto ahora y ábrela con `positron .` cuando llegues allí.

  Una vez instalado, los comandos `ws`, `positron`, `uv`, `git` funcionan como palabras sueltas porque están en tu **PATH** — la lista de carpetas donde la terminal busca ejecutables. Si alguno dice "command not found", en [IDE & Verificación](#paso-12) tienes la guía completa para ver tu PATH y agregar rutas manualmente en Windows y Mac.
</Callout>
